...
exports.es = {
    ...
    maxLength: (n) => `El campo no puede tener más de ${n} caracteres.`,
    exactLength: (n) => `El campo debe tener exactamente ${n} caracteres.`,
    ...
    contains: (v) => `El campo debe contener "${v}".`,
    startsWith: (v) => `El campo debe comenzar con "${v}".`,
    endsWith: (v) => `El campo debe terminar con "${v}".`,
    ...
    numberRange: (min, max) => `El valor debe estar entre ${min} y ${max}.`,
    numberPositive: 'Solo se permiten números positivos.',
    numberNegative: 'Solo se permiten números negativos.',
    integer: 'El campo debe ser un número entero.',
    multipleOf: (n) => `El valor debe ser múltiplo de ${n}.`,
    // Date
    dateFormat: (format) => `El formato de fecha no es válido. El formato esperado es (${format}).`,
    minDate: (v) => `La fecha debe ser el ${v} o posterior.`,
    maxDate: (v) => `La fecha debe ser el ${v} o anterior.`,
    ...
    fileDimensions: (w, h) => `Las dimensiones del archivo deben ser ${w}x${h}.`,
    imageAspectRatio: (w, h) => `La relación de aspecto de la imagen debe ser ${w}:${h}.`,
    imageMinDimensions: (w, h) => {
        const parts = [];
        if (w !== undefined)
            parts.push(`ancho >= ${w}px`);
        if (h !== undefined)
            parts.push(`alto >= ${h}px`);
    ...
    // --- Nuevos v2.1 ---
    // Numeric
    greaterThan: (n) => `El valor debe ser mayor que ${n}.`,
    lessThan: (n) => `El valor debe ser menor que ${n}.`,
    precision: (n) => `El valor no puede tener más de ${n} decimales.`,
    // Date
    dateAfter: (v) => `La fecha debe ser posterior a ${v}.`,
    dateBefore: (v) => `La fecha debe ser anterior a ${v}.`,
    // Enum / set
    oneOf: (values) => `El valor debe ser uno de: ${values.join(', ')}.`,
    // Cross-field
    notMatchField: 'Los campos no deben coincidir.',
    requiredUnless: 'Este campo es obligatorio.',
    // Array
    arrayMinLength: (n) => `El array debe tener al menos ${n} elementos.`,
    arrayMaxLength: (n) => `El array no puede tener más de ${n} elementos.`,
    arrayUnique: 'El array no debe contener valores duplicados.',
    arrayContains: (v) => `El array debe contener "${v}".`,
    // Format
